...
router.route('/registerStartup').post((req, res) => {
    ...
    startup_1.default.collection.insertOne({ 'username': username, 'fullName': fullName, 'establishmentDate': establishmentDate,
...
router.route('/registerInvestor').post((req, res) => {
    ...
    let logo = req.body.logo;
    investor_1.default.collection.insertOne({ 'username': username, 'fullName': fullName, 'establishmentDate': establishmentDate,
        'registrationNumber': registrationNumber, 'taxId': taxId, 'firstName': firstName, 'middleName': middleName, 'lastName': lastName,
        'address': address, 'municipality': municipality, 'city': city, 'country': country, 'phoneNumber': phoneNumber, 'email': email,
...
router.route('/addStartupInterest').post((req, res) => {
    ...
    startup_1.default.collection.updateOne({ 'username': username }, { $push: { 'interests': interest } });
    res.json({ message: 1 });
});
router.route('/deleteStartupInterest').post((req, res) => {
    let username = req.body.username;
    let interest = req.body.interest;
    startup_1.default.collection.updateOne({ 'username': username }, { $pull: { 'interests': interest } });
    res.json({ message: 1 });
});
router.route('/addStartupSkill').post((req, res) => {
    let username = req.body.username;
    let skill = req.body.skill;
    startup_1.default.collection.updateOne({ 'username': username }, { $push: { 'professionalSkills': skill } });
    res.json({ message: 1 });
});
