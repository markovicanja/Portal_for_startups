...
export class AddUsersComponent implements OnInit {
  ...
  registerAdmin() {
    this.msgAdmin = "";
    if (!this.usernameAdm || !this.pass1Adm || !this.pass2Adm) {
      this.msgAdmin = "Please fill in all the fields.";
    }
    this.service.getUser(this.usernameAdm).subscribe((user: User) => {
      if (user) {
        this.msgAdmin = "Username already exists.";
        return;
      }
      else {
        if (this.pass1Adm != this.pass2Adm) {
          this.msgAdmin = "Passwords must be same.";
          return;
        }
        this.service.registerAdmin(this.usernameAdm, this.pass1Adm).subscribe(() => {
          this.router.navigate(['profile']);
        }); 
      }
    });
